...
use crate::{capi::plugin, command::{Command, Response, ResponseError}, event::Events, plugin::Plugin};
...
impl PluginLoader {
    ...
    pub fn load_plugin(&mut self, path: &str) -> Result<(), Box<dyn Error>> {
        unsafe {
            ...
            // ctor(&mut plugin);
            let ctor: Symbol<fn(Events) -> Box<dyn Plugin>> = lib.get(b"create_plugin")?;
            let plugin = ctor(Events::new());
